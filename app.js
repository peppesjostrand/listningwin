...
// ═══════════════════════════════════════════════
const BRAND_COLORS = ['#a78bfa','#f472b6','#0D4F35','#facc15','#4ade80','#38bdf8','#f87171','#34d399','#e879f9','#94a3b8'];
...
const CHAIN_COLORS = { coop: '#4ade80', ica: '#f87171', dagab: '#0D4F35' };
      ...
      ${lanseringar.map(l => {
        ...
        return `<div class="lansering-card ${selectedLanseringId===l.id?'selected':''}" onclick="selectLansering('${l.id}')">
          ...
          <div class="lansering-card-meta" style="margin-top:6px">
            ...
            ${l.chain ? `<span style="color:${l.chain==='coop'?'#4ade80':l.chain==='ica'?'#f87171':'#0D4F35'}">${l.chain==='coop'?'Coop':l.chain==='ica'?'ICA':l.chain==='dagab'?'Dagab':'Flera'}</span>` : ''}
          </div>
        </div>`;
      }).join('')}
  ...
  if (step >= 2) {
    ...
    if (step >= 3 && wizardData.chains.length)
      parts.push(...wizardData.chains.map(c => {
        const col = c === 'coop' ? '#4ade80' : c === 'ica' ? '#f87171' : '#0D4F35';
...
function buildWizardCatSection(chain) {
  const col = chain === 'coop' ? '#4ade80' : chain === 'ica' ? '#f87171' : '#0D4F35';
...
function renderDashWidgets() {
  ...
  el.innerHTML = `<div class="dash-widgets">
    ...
    <div class="dash-widget" onclick="showTab('lansering')">
      <div class="dash-widget-value" style="color:${openTasks>0?'#0D4F35':'#4ade80'}">${openTasks}</div>
...
function renderAgenda() {
  ...
  visibleRounds().filter(r => daysLeft(r.launchDate) >= -7 && daysLeft(r.launchDate) <= 90).forEach(r => {
    allItems.push({
      ...
      color: r.source === 'coop' ? '#4ade80' : r.source === 'ica' ? '#f87171' : '#0D4F35',
      type: 'window',
      readonly: true
    });
  });
...
function renderBrandLanseringar(brand) {
  ...
  const chainColors = { coop: '#4ade80', ica: '#f87171', dagab: '#0D4F35', multi: '#a78bfa' };
...
function renderBrandWindows(brand, allCats) {
  ...
  const chainColors = { coop: '#4ade80', ica: '#f87171', dagab: '#0D4F35' };
